...
def init_qa_chain(repo_dir: str):
    ...
    # Load documents (uses @st.cache_data)
    ...
    st.write("Vector store ready.") # Optional: Add user feedback
...
if "qa_chain" not in st.session_state:
    # Display spinner message specific to chain initialization
...
if st.button("Send") and query:
    ...
    if source_docs:
        ...
        # Display unique sources (remains outside the expander)
        unique_sources = set(doc.metadata.get("source", "Unknown Source") for doc in source_docs)
        for source in sorted(list(unique_sources)):
             st.markdown(f"- `{source}`")

        # Use st.expander for the detailed snippets, closed by default
        with st.expander("Show Source Document Snippets", expanded=False):
            # This code will now be inside the collapsible section
            # st.markdown("---") # You might not need the separator anymore, or keep it inside
            for i, doc in enumerate(source_docs):
                meta = doc.metadata
                source_file = meta.get('source', 'Unknown Source')
                content = doc.page_content[:200] + "..." if len(doc.page_content) > 200 else doc.page_content
                
                # Display source and truncated content inside the expander
                st.markdown(f"**Snippet {i+1} ({source_file}):**")
                st.markdown(f"```\n{content}\n```") # Use code block for better visual separation
